...
export const ResourceCategoryUpdate: React.FC<PropInterface> = ({
  ...
  const getParams = () => {
    resourceCategory.createResourceCategory().then((res: any) => {
      ...
      if (JSON.stringify(categories) !== "{}") {
        ...
        new_arr.unshift({
          label: "As a first class category",
          value: 0,
        });
  ...
  const displayRender = (label: any, selectedOptions: any) => {
    if (selectedOptions && selectedOptions[0]) {
      ...
      if (current === id) {
        message.error("Cannot choose itself as a parent class");
        return "无";
      }
    }
        ...
        <div className="float-left mt-24">
          ...
          >
            ...
            <Form.Item
              label="categories名称"
              name="name"
              rules={[{ required: true, message: "Please entercategories名称!" }]}
            >
              <Input
                style={{ width: 200 }}
                allowClear
                placeholder="Please entercategories名称"
              />
            </Form.Item>
          </Form>
        </div>
