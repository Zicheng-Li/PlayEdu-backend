...
const MemberImportPage = () => {
  ...
  const storeBatchTableData = (data: any) => {
    user
      ...
      .then(() => {
        setErrorData([]);
        message.success("Importsuccessfully！");
        navigate(-1);
      })
      .catch((e) => {
        if (e.code === -1) {
  ...
  const download = () => {
    let url = getHost() + "template/student Batch Import Templates.xlsx";
    window.open(url);
  };
      ...
      <Row className="playedu-main-body">
        <Col>
          <div className="float-left mb-24">
            <BackBartment title="student batch Import" />
          </div>
          <div className="float-left d-flex  mb-24">
            ...
            Download "Student Batch Import Template"
            </Button>
          </div>
